...
# dependencies = []
# ///
"""email_pipeline_promptguard — Ductile plugin (protocol v2).

Second independent scorer in the ductile email pipeline. Delegates
classification to the PromptGuard 2 HTTP service (promptguard-service
running on Unraid). Sends each email part as a POST /score request,
aggregates per-part scores, emits email.scored.promptguard.
...
Event emitted: email.scored.promptguard
  ...
  dedupe_key                — promptguard-scored:msg:<message_id>

Config keys (all optional):
  endpoint_url        (str, default http://192.168.20.4:11441/score)
...
EVENT_TYPE = "email.scored.promptguard"
DEDUPE_PREFIX = "promptguard-scored:msg:"
DEFAULT_ENDPOINT = "http://192.168.20.4:11441/score"
DEFAULT_HEALTH_ENDPOINT = "http://192.168.20.4:11441/health"
...
class PromptGuardScorePayload(TypedDict):
    message_id: str
    verdict: str
    score: float
    decision: str
...
class PromptGuardEvent(TypedDict):
    type: str
    payload: PromptGuardScorePayload
    dedupe_key: str
...
class ResponseOk(TypedDict):
    ...
    result: str
    logs: list[LogEntry]
    events: NotRequired[list[PromptGuardEvent]]
...
def ok(
    ...
    *,
    ...
    events: list[PromptGuardEvent] | None = None,
) -> ResponseOk:
    resp: ResponseOk = {
        "status": "ok",
        "result": result,
...
def _score_text(
    ...
    """POST text to the PromptGuard service. Returns (score, decision, reason)."""
    ...
    try:
        ...
        raise RuntimeError(f"HTTP {exc.code} from promptguard service: {exc.reason}") from exc
    except urllib.error.URLError as exc:
        raise RuntimeError(f"Cannot reach promptguard service at {endpoint}: {exc.reason}") from exc
    except json.JSONDecodeError as exc:
        raise RuntimeError(f"Non-JSON response from promptguard service: {exc}") from exc
...
def cmd_health(config: dict[str, Any]) -> ResponseOk | ResponseErr:
    ...
    try:
        ...
        return err(f"promptguard service unreachable at {health_url}: {exc}", retry=False)
    except json.JSONDecodeError as exc:
        return err(f"non-JSON health response: {exc}", retry=False)

    if body.get("status") != "ok":
        return err(f"promptguard service unhealthy: {body}", retry=False)

    model = body.get("model", "unknown")
    return ok(f"email_pipeline_promptguard healthy — service reports model={model}")
